...
/// @notice Non-upgradeable mock of the ERC-8004 Reputation Registry, for local/unit testing only.
/// @dev Mirrors erc-8004/erc-8004-contracts' ReputationRegistryUpgradeable, minus the UUPS/Ownable
/// upgrade machinery and ERC-7201 namespaced storage (unnecessary without a proxy).
contract MockReputationRegistry is IReputationRegistry {
        ...
        returns (uint64 count, int128 summaryValue, uint8 summaryValueDecimals)
    {
        ...
        for (uint256 i; i < clientList.length; i++) {
            ...
            for (uint64 j = 1; j <= lastIdx; j++) {
                ...
                // Normalize to 18 decimals (WAD). valueDecimals is bounded to <= 18 on write.
        ...
        returns (uint64 count)
    {
        if (clientAddress == address(0)) {
            ...
            for (uint256 i; i < clientsList.length; i++) {
                uint64 lastIdx = _lastIndex[agentId][clientsList[i]];
                for (uint64 j = 1; j <= lastIdx; j++) {
                    count += _countResponses(agentId, clientsList[i], j, responders);
                }
            }
